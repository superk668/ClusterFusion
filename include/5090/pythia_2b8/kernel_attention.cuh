/**
 * ClusterFusion Pythia-2.8B Attention + MLP Up Kernel (Split Version)
 * 
 * Handles: LayerNorm → QKV → RoPE → Flash Decoding → Output Projection 
 *          → Post-LN → MLP Up → GELU → Write mlp_intermediate
 * 
 * This kernel replaces the first part of the fused kernel up to MLP Up.
 * The MLP Down is handled by a separate kernel (kernel_mlp.cuh).
 * 
...
__forceinline__ __device__ float ptx_exp2_attn(float x) {
    float y;
    asm volatile("ex2.approx.ftz.f32 %0, %1;" : "=f"(y) : "f"(x));
    return y;
}

// Fast tanh using PTX exp2
__forceinline__ __device__ float ptx_tanh_attn(float x) {
    if (x > 10.0f) return 1.0f;
    if (x < -10.0f) return -1.0f;
    constexpr float LOG2_E = 1.44269504088896340736f;
    float exp_2x = ptx_exp2_attn(2.0f * x * LOG2_E);
    return (exp_2x - 1.0f) * __frcp_rn(exp_2x + 1.0f);
}

// Fast GELU using PTX
__forceinline__ __device__ float ptx_gelu_attn(float x) {
    constexpr float GELU_CONST = 0.7978845608028654f;
    constexpr float GELU_COEF = 0.044715f;
    float x3 = x * x * x;
    float inner = GELU_CONST * (x + GELU_COEF * x3);
    return x * 0.5f * (1.0f + ptx_tanh_attn(inner));
}

__global__ void __cluster_dims__(CLUSTER_SIZE, 1, 1) PythiaAttentionMlpUpKernel(
    // Output tensors
    half* attn_output,      // [1, HIDDEN_DIM] - attention output (with bias, for residual)
    ...
    // Attention weights
    half* qkv_bias,         // [3 * NUM_HEADS * HEAD_DIM]
    ...
    half* post_ln_buffer,   // [HIDDEN_DIM] - for MLP up input
    // TMA descriptors
    ...
    const float softmax_scale = 0.16129820913429784f;
    ...
    for (int id = 1; id < KV_DIM_PER_BLOCK / TMA_LOAD_ONCE_ATTN; id++) {
        ...
        scale = (local_max > -CUDART_INF_F) ? ptx_exp2_attn(pre_max - local_max) : 0.0f;
        local_sum *= scale;
        #pragma unroll
        for (int j = 0; j < DEC_TILE; j++) {
            int seq_idx = KV_DIM_PER_BLOCK * cluster_block_id + (id - 1) * TMA_LOAD_ONCE_ATTN + weight_idx_2 + j;
            if (seq_idx < SEQ_LEN) {
                qk[j] = ptx_exp2_attn(qk[j] - local_max);
                local_sum += qk[j];
            } else {
                qk[j] = 0.0f;
            }
    ...
    if (cluster_block_id == 0 && warp_id == 0) {
        ...
        if (lane_id / NUM_THREAD_PER_ROW_2 == 1) {
            ...
            scale = (pre_max > -CUDART_INF_F) ? ptx_exp2_attn(pre_max - local_max) : 0.0f;
            local_sum *= scale;
            qk[0] = ptx_exp2_attn(qk[0] - local_max);
            local_sum += qk[0];
            #pragma unroll
            for (int j = 0; j < NUM_PER_THREAD; j++) {
                reg_reduce[j] = reg_reduce[j] * scale;
    ...
    // Store partial attention output
    #pragma unroll
    for (int i = 0; i < NUM_PER_THREAD; i++) {
    ...
    for (int j = 0; j < DIM_BLOCK_REDUCE / 2; j++) {
        ...
        scale = (m > -CUDART_INF_F && local_max > -CUDART_INF_F) ? ptx_exp2_attn(m - local_max) : 0.0f;
        s *= scale;
        float rescale = (pre_max > -CUDART_INF_F && local_max > -CUDART_INF_F) ? ptx_exp2_attn(pre_max - local_max) : 0.0f;
    ...
    // Cluster-level reduction for attention
    pre_max = local_max;
    if (tid == 0) {
    ...
    scale = (pre_max > -CUDART_INF_F && cluster_local_max > -CUDART_INF_F) ? ptx_exp2_attn(pre_max - cluster_local_max) : 0.0f;
    local_sum *= scale;
    #pragma unroll
    for (int j = 0; j < NUM_PER_THREAD; j++) {
        reg_reduce[j] = reg_reduce[j] * scale;
    ...
    for (int id = 1; id < DIM_PER_BLOCK / TMA_LOAD_ONCE; id++) {
        ...
        if (lane_id % NUM_THREAD_PER_ROW_3 == 0) {
            atomicAdd(&attn_output[cluster_block_st_id + weight_idx_3 + (id - 1) * TMA_LOAD_ONCE], __float2half(tmp));
        }
        block.sync();
    }
    bar[(DIM_PER_BLOCK / TMA_LOAD_ONCE - 1) % 2].wait(std::move(token[(DIM_PER_BLOCK / TMA_LOAD_ONCE - 1) % 2]));
    ...
    if (lane_id % NUM_THREAD_PER_ROW_3 == 0) {
        atomicAdd(&attn_output[cluster_block_st_id + weight_idx_3 + ((DIM_PER_BLOCK / TMA_LOAD_ONCE) - 1) * TMA_LOAD_ONCE], __float2half(tmp));
    }
    ...
    for (int d = tid; d < DIM_PER_BLOCK; d += BLOCK_SIZE) {
        ...
        if (head_id == 0) {
            atomicAdd(&attn_output[out_idx], o_bias[out_idx]);
        }
    }
    ...
    // Apply GELU and write to global memory
    if (lane_id % NUM_THREAD_PER_ROW == 0 && weight_idx < HEAD_DIM) {
        float val = tmp + __half2float(mlp_up_bias[ffn_block_offset + weight_idx]);
        mlp_intermediate[ffn_block_offset + weight_idx] = __float2half(ptx_gelu_attn(val));
    }
    
    // No grid.sync() here - kernel boundary provides synchronization
}
