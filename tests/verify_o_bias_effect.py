"""
Verify that adding o_bias in Python gives correct results
"""
import torch
...
def pytorch_attn_with_bias(hidden_states, ln_weight, ln_bias, qkv_weight, qkv_bias, o_weight, o_bias,
                            k_cache, v_cache, cos, sin, num_heads=32, head_dim=80, rotary_dim=20):
    """PyTorch attention WITH bias"""
    ...
    attn_output_with_bias = F.linear(attn_output, o_weight, o_bias)
    
    return attn_output_with_bias
...
# PyTorch with bias (ground truth)
with torch.no_grad():
    attn_pytorch_with_bias = pytorch_attn_with_bias(
        hidden_states.clone(),
        weights['ln_weight'],
        weights['ln_bias'],
        weights['qkv_weight'],
...
print(f"\n✓ PyTorch WITH bias (ground truth):")
print(f"  norm: {attn_pytorch_with_bias.norm().item():.4f}")
print(f"  first 8: {attn_pytorch_with_bias[0, :8]}")

# Kernel (no bias) + manually add bias in Python
with torch.no_grad():
    attn_kernel_no_bias, _, _ = clusterfusion.pythia_decoder_layer(
        hidden_states.clone(),
        weights['qkv_weight'],
        weights['qkv_bias'],
        weights['o_weight'],
    ...
    # Add bias manually (like test_pythia_with_kernel.py does)
    attn_kernel_with_bias = attn_kernel_no_bias + weights['o_bias'].view(1, -1)

print(f"\n✓ Kernel + manually added bias:")
print(f"  norm: {attn_kernel_with_bias.norm().item():.4f}")
print(f"  first 8: {attn_kernel_with_bias[0, :8]}")

# Compare
diff = (attn_pytorch_with_bias - attn_kernel_with_bias).abs()
print(f"\nDifference:")
print(f"  Max error: {diff.max().item():.4f}")
print(f"  Mean error: {diff.mean().item():.4f}")
print(f"  Relative error: {(diff.mean() / attn_pytorch_with_bias.abs().mean()).item():.4f}")

if diff.mean().item() < 1.0:
    print(f"\n✓ Adding o_bias in Python gives correct results!")
else:
    print(f"\n✗ Still large error even with o_bias added")
